...
var WorkbookReader = module.exports = function (options) {
  ...
  this.styles.init();
  this.properties = new WorkbookPropertiesManager(); // worksheet readers, indexed by sheetNo

  this.worksheetReaders = {}; // hyperlink readers, indexed by sheetNo

  this.hyperlinkReaders = {}; // count the open readers

  this.readers = 0; // end of stream check

  this.atEnd = false; // worksheets, deferred for parsing after shared strings reading

  this.waitingWorkSheets = []; // callbacks for temp files cleanup
...
utils.inherits(WorkbookReader, events.EventEmitter, {
  ...
  read: function read(input, options) {
    ...
    zip.on('entry', function (entry) {
      var match;
      var sheetNo; // console.log(entry.path);
